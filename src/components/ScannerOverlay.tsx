...
export const ScannerOverlay: React.FC<ScannerOverlayProps> = ({
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (permStatus.camera !== 'granted') {
          ...
          if (req.camera !== 'granted') {
            if (isMounted) {
              setHasCamera(false);
              setCameraError('Camera permission was denied in Android settings. Please enable camera in device settings.');
    ...
    async function initWebCamera() {
      try {
        setScanStatus('Checking camera hardware...');
        // Request temporary stream to trigger browser permissions prompt and populate device list
        if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
          try {
            ...
            if (permErr?.name === 'NotAllowedError' || permErr?.name === 'PermissionDeniedError') {
              if (isMounted) {
                setHasCamera(false);
                setCameraError('Camera permission was blocked. Tap the lock/camera icon in your browser address bar to allow access.');
                setScanStatus('⚠️ Camera access denied. Use photo upload or manual key-in below.');
              }
              return;
            }
          }
